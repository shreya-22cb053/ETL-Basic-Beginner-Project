...
SELECT TOP 10 product_id, SUM(sales_price * quantity) AS revenue
FROM df_orders
GROUP BY product_id
...
WITH CTE1 AS
(SELECT category, FORMAT(order_date,'yyyy-MM') AS order_year_month, SUM(sales_price*quantity) AS category_sales 
FROM df_orders
...
SELECT TOP 1 *, ROUND((profit_2023-profit_2022)*100/profit_2022,2) as YoY_change
FROM CTE2
